...
bool robot_finder(const cv::Mat& img_in, const double scale, Polygon& triangle, double& x, double& y, double& theta){
  ...
  {
    cv::approxPolyDP(robot_contours[i], approx_robot, 15, true); //between 4 and 23
    //we have to be sure the robot was found, aka the triangle
    if (approx_robot.size() != 3)
      continue;
    robot_found = true;
    //std::cout << approx_robot.size() << std::endl;
  ...
  //Calculate radus of robot so we can calculate the inflation
  float maxt = 0;
  for (auto item : triangle)
  {
