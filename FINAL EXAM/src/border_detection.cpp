...
bool border_detection(const cv::Mat& hsv_img, const double scale, std::vector<Polygon>& obstacle_list){
  ...
  cv::inRange(hsv_img, cv::Scalar(0, 0, 0), cv::Scalar(180, 255, 30), border_mask); // -----------------------------------SECOND RANGE TO 180 180 50 FOR REAL IMAGE
  ...
  for (int i = 0; i < border_contours.size(); ++i)
  {
    ...
    if (cv::contourArea(border_contours[i]) > 3000)  // ------------------------------------------7000 FOR REAL IMAGE
    {
      //we reduce the number of vertices , 3 was the best option 
      cv::approxPolyDP(border_contours[i], approx_border, 3, true); // -------------------------------5 in the real world image
