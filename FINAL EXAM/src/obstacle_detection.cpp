...
bool obstacle_detection(const cv::Mat& hsv_img, const double scale, std::vector<Polygon>& obstacle_list){
  ...
  cv::inRange(hsv_img, cv::Scalar(164, 102, 80), cv::Scalar(180, 255, 255), red_mask_high);
  ...
  for (int i = 0; i < obstacles_contours.size(); ++i)
  {
    ...
    std::cout << "Obstacle number " << i+1 << " with vertices " << approx_obstacles.size() << "  area " << cv::contourArea(approx_obstacles) << std::endl;
